...
for (const seed of seeds) {
  const v = await p.evaluate(async ([seed, mode, size, style, days, over]) => {
    ...
    try {
      if (over) new Function(over)(); const _bp = G.breakPeace.bind(G); G.breakPeace = function () { if (S.peace) out.peaceBrokenAt = { day: S.day, stack: String(new Error().stack).split("\n").slice(1, 7).join(" | ") }; return _bp(); };
      ...
      const bot = () => {
        ...
        if (res.food < 150) want.push(T.FERTILE);
        ...
        if (Bld.list('P').filter(b => b.key === 'farm').length < 2) lay('farm', 'on', T.BARREN);
