/* MOUNTAIN CONTRACT — a mountain is an OBJECT, not a grid of tiles
   ...
   4. IT NEVER LEAVES ITS OWN GROUND BY MORE THAN MTN.OUT_MAX. The art may sit
      off the lattice; it may not wander a tile into a meadow.
...
/* ---- 2. THE SILHOUETTE ------------------------------------------------ */
{
  ...
  const v = await p.evaluate(new Function(boot + `
    ...
    // …and it never leaves its own ground: sample the drawn art and check
    // every opaque pixel against the true tile footprint
    const outside = R.mtnOutsideFn();
    const TL = CFG.TILE;
    let far = 0, opaque = 0, worst = 0;
    for (const a of R._mtnArt) {
      ...
      for (let j=0;j<a.c.height;j+=2) for (let i=0;i<a.c.width;i+=2) {
        ...
        opaque++;
        const o = outside((a.x+i+0.5)/TL, (a.y+j+0.5)/TL);
        if (o > worst) worst = o;
        if (o > MTN.OUT_MAX + 0.02) far++;
      }
    }
    return { m, c, far, opaque, worst: +worst.toFixed(3) };`));
  ...
  ck('andItNeverLeavesItsOwnGround', v.far === 0,
    v.opaque + ' sampled rock pixels, the furthest ' + v.worst
    + ' tiles outside the footprint (bound ' + 0.22 + ')');
  await p.close();
}
...
/* ---- 4. GAMEPLAY TRUTH IS TILE-BASED --------------------------------- */
{
  ...
  const v = await p.evaluate(new Function(boot + `
    ...
    const bm = R.compositeMtn, bo = R.buildMtnLayer;
    R.compositeMtn = () => {}; R.buildMtnLayer = () => {}; R.rebuildTerrain();
    const off = { ans: answers(), sig: sigMap() };
    R.compositeMtn = bm; R.buildMtnLayer = bo; R.rebuildTerrain();
