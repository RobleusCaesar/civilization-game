...
const R = {
  ...
  /* THE APRON IS GONE, AND IT IS TERRAIN NOW (map.js, "A MOUNTAIN STANDS IN
     ITS OWN WOOD"). A massif used to grow its skirt HERE: mtnEdgeTrees
     stamped art-only stands on the open ground at the rock's foot so the
     mountain did not read as dropped onto bare meadow. They blocked nothing
     and yielded nothing — and because the strips below are interleaved with
     the units by ground row, a stand at or south of a unit's feet DREW OVER
     that unit. The operator, twice: "it looks like a forest" and then "look
     they're walking under the trees on this map". They were.
     The skirt is planted in map generation now as real T.FOREST: it blocks,
     an axe fells it for wood, and it bakes into the terrain cache UNDER
     every unit like every other wood. Nothing draws trees here any more —
  ...
  drawMtnChained(r) {
    ...
    /* the composite below measures ROCK, and takes it from the strips as they
       ...
       the snapshot necessary — sorted in by row among the rock they pushed
       rock out of the composite, and the honesty pin then measured tree
       crowns on the meadow as stray rock. The wood is terrain now and no
       longer arrives here at all, but the snapshot stays: it also holds the
       strips in PLACEMENT order, which is the order the pieces overlap in. */
