...
const AI = {
  ...
  seenGuns() {
    /* NOBODY IS SHELLING US DURING A TRUCE (tests/calm-peace.mjs). This read
       drives a strategic reaction — commanding build utilities for the dock,
       the workshop and the hall, engines and hulls trained ahead of the
       wall-breaking quota, and answerTheGuns walking the battery out to
       firing position — off the mere PRESENCE of a player ranged unit on
       ground the chief cannot walk to. At peace that is a Calm player's
       archer standing in their own country, and the chief was rebuilding its
       whole economy around an attack that is not happening, then marching
       engines at a town it is not at war with. The funnel, not a hand-rolled
       owner check: the guns are real again the moment the player strikes. */
    if (!Combat.hostile('A', 'P')) return [];
    const reach = this.aiLandReach();
    const out = [];
    for (const u of S.units) {
  ...
  armyWant(m, post) {
    // the cap climbs one spear every aiRampDays past day 60. EVERY mode
    // declares it (Moderate 18, the others 12) — it used to be read here
    // with a `|| 12` that no mode ever overrode, so the gentler Moderate
    // ramp this comment promised silently did not exist.
  ...
  daily() {
    ...
    if (Units.count('A', u => Units.isVillager(u)) < wantV) {
      ...
      if (ai.purse >= 50) {
        ...
        // the funnel on the P clause (tests/calm-peace.mjs): at peace a
        // player soldier standing near the rival hall is a visitor, not a
        // kill zone, and an unscreened chief that banked its purse over one
        // would stop hiring villagers for the rest of a Calm run — stalling
        // the wonder race the sibling threat read is gated to protect.
        // Barbarians are outside the truce and still stop the hiring.
        const foes = S.units.filter(u => ((u.owner === 'P' && Combat.hostile('A', 'P')) || u.owner === 'R') &&
    ...
    /* ---- repair crews: chip damage must not accumulate forever. Any
       ...
       AND YES, THIS IS ONE-SIDED, for the reason Combat.bestFoe is: the
       player can ORDER a repair and a chief cannot be told to. The player's
       own mending is CFG.REPAIR_RATE — half a building's hp for a day of a
       villager's time, ten times this rate — so the trade is fast-but-costs-
       labour against free-but-slow, not a gift. Flagged as an undocumented
       asymmetry in a symmetry audit; it is deliberate, and now it says so. */
    for (const b of Bld.list('A')) {
